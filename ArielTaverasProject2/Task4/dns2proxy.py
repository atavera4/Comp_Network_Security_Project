...
import argparse
#Ariel Taveras. Added this import statement below.
...
#Added by Ariel Taveras.
#This will store the resolved ip addresses of visited sites.
Resolved_IP_Address = list("")
Resolved_IP_Address.append("START")
Resolved_Spoof_Address = list("")
Resolved_IP_Counter = 0
Resolved_Spoof_Counter = 0
Resolve_New_Request = 0

######################
# GENERAL SECTION    #
######################

#Ariel Taveras. Added this function below.
#Create a new file for the spoof.cfg without #'s
def CheckIfBlacklisted(site):
    file = open("spoof.cfg", 'r')
    file_spoof = open("DNS_spoof_resolved", 'w')
    for line in file:
        if (filter(lambda x: x.isalpha(), "".join(line).replace("www.",'')) in filter(lambda x: x.isalpha(), "".join(site).replace("www.",''))):
	    #print the first few positions of the site and then the spoof.cfg line.
            file_spoof.write("".join(line))
            return 0
    return 1;

#Ariel Taveras. Finds the spoofed address.
# here < site > is the str1 from the "save_req" function.
def FindSpoofAddress(site):
    f = open("spoof.cfg", 'r')
    filtered_line = list("")
    for line in f:
        if (FilterSpoofFile(line) in site.replace("www.", '')):
            temp_line = GetSpoofIP(line)
            for i in temp_line:
                if(i == '\n'):
...
def FilterSpoofFile(line):
    ...
    for i in line:
        if (i == ' '):
            break
        temp_list.append(i)
    return "".join(temp_list).replace("www.", ' ')

# Ariel Taveras. Edited this function below.
def save_req(lfile, str1):
    f = open(lfile, "a")
    f2 = open("dnslog_ipAddress_resolved", 'a')
    global Resolved_IP_Counter
    global Resolve_New_Request
    global Resolved_IP_Address
    global Resolved_Spoof_Address
    global Resolved_Spoof_Counter
    #Ariel Taveras. Added the %f for milliseconds below.
    #also added code that would not print out the blocked sites to the dns log.
    if(CheckIfBlacklisted(str1)): # TASK 2A.
        Resolve_New_Request = 1
        #f2.write(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f") + ' ' + str1)
        f.write(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f") + ' ' + str1.strip('\n') + ' ' + str(Resolved_IP_Address[Resolved_IP_Counter]) + '\n')
    else:
        f2.write(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f") + ' ' + str1.strip('\n') + ' ' + FindSpoofAddress(str1.replace("AAAA", '')) + '\n')
...
def std_A_qry(msg, prov_ip):
    ...
    for q in qs:
        ...
        #Ariel Taveras. Added/Edited lines below.
        # ONLY DO THIS FOR THE NEW REQUESTS !!!!!!!
        global Resolve_New_Request
        global Resolved_IP_Address
        global Resolved_IP_Counter
        for realip in ips:
            DEBUGLOG('Adding real IP  = ' + realip.to_text())
	    if(Resolve_New_Request):
                Resolved_IP_Address.append(realip.to_text())
                Resolved_IP_Counter += 1
                Resolve_New_Request = 0
